...
//       if (lines.length < 2) return; // Захист від порожніх файлів

//       // --- ДИНАМІЧНИЙ ПАРСЕР КОЛОНОК ---
//       // Читаємо заголовки і шукаємо індекси потрібних колонок (беремо останні входження, бо таблиці здвоєні)
//       const headers = lines[0].split(';').map(h => h.replace(/^"|"$/g, '').trim().toLowerCase());
      
//       const dataIdx = headers.lastIndexOf('data');
//       const hoursIdx = headers.lastIndexOf('godziny');
      
//       // Шукаємо колонку причини (вона може називатися по-різному)
//       let reasonIdx = headers.lastIndexOf('powód');
//       if (reasonIdx === -1) reasonIdx = headers.lastIndexOf('komentarz');
//       if (reasonIdx === -1) reasonIdx = headers.lastIndexOf('comment');

//       // Фоллбек на старі індекси, якщо заголовки зовсім відсутні або пошкоджені
//       const safeDataIdx = dataIdx > -1 ? dataIdx : 5;
//       const safeHoursIdx = hoursIdx > -1 ? hoursIdx : 6;
//       const safeReasonIdx = reasonIdx > -1 ? reasonIdx : 7;
//       // ----------------------------------
...
//         // Захист від битих/коротких рядків
//         if (columns.length <= safeHoursIdx) continue;

//         const dateStr = columns[safeDataIdx];
//         const hoursStr = columns[safeHoursIdx];
//         const reason = columns[safeReasonIdx] || 'Без коментаря';
